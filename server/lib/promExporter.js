...
module.exports = async function(rooms, peers, config)
{
	const collect = async function(registry)
	{
		...
		const mRooms = new prom.Gauge({ name: 'livechart_rooms', help: '#rooms', registers: [ registry ] });

		mRooms.set(rooms.size);
		const mPeers = new prom.Gauge({ name: 'livechart_peers', help: '#peers', labelNames: [ 'room_id' ], registers: [ registry ] });
